...
const GitHubStats = () => {
  ...
  const loadGitHubData = async () => {
    try {
      ...
      setStats(statsRes.data);
      setRepos(reposRes.data.slice(0, 6)); // Show top 6 repos
    } catch (err) {
      console.error('Failed to load GitHub data:', err);
    } finally {
      setLoading(false);
      ...
      <div className="container-custom">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <p className="mt-4 text-light-gray/60">Fetching GitHub data...</p>
          </div>
        ) : (
          <div className="space-y-12">
            {/* Stats Cards */}
            <motion.div
              ...
              className="grid grid-cols-2 md:grid-cols-4 gap-6"
            >
              <div className="glass p-6 rounded-xl text-center hover:shadow-xl hover:shadow-accent/10 transition-all duration-300 group">
              ...
              <div className="glass p-6 rounded-xl text-center hover:shadow-xl hover:shadow-accent/10 transition-all duration-300 group">
                ...
                <div className="text-light-gray/70 flex items-center justify-center gap-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                {stats?.topLanguages?.map((lang, index) => (
                  <motion.div
                    ...
                    className="glass p-4 rounded-xl flex items-center gap-3 hover:shadow-lg transition-all group"
                  >
                    <div
                      className="w-4 h-4 rounded-full shadow-lg group-hover:scale-110 transition-transform"
                      style={{
                        backgroundColor: languageColors[lang.language] || languageColors.default,
                      }}
                    />
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {repos.map((repo, index) => (
                  <motion.a
                    ...
                    className="glass p-6 rounded-xl card-hover group cursor-pointer block transition-all duration-300"
                  >
                    <div className="flex items-start justify-between mb-3">
                      <h4 className="font-bold text-lg group-hover:text-accent transition-colors line-clamp-1">
                        {repo.name}
                      </h4>
                      <div className="flex items-center gap-1 text-light-gray/70 bg-secondary-dark/50 px-2 py-1 rounded-full">
                        <svg className="w-4 h-4 text-yellow-500" fill="currentColor" viewBox="0 0 24 24">
                          <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                        </svg>
                        {repo.stargazers_count}
                    ...
                    <p className="text-light-gray/70 text-sm mb-4 line-clamp-2 min-h-[40px]">
                      {repo.description || 'No description available'}
                    </p>
                    <div className="flex items-center gap-4">
                      {repo.language && (
